...
class Q_learning():
	def __init__(self, tetris):
		...
		self.q_values = np.zeros((20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 7, 40))
				=	2293760 petabyte
		'''
		#				media da altura das colunas, 7 tetrominos,  
		#							 3 ações(Minimizar  buracos, Minimizar altura, Maximizar linhas)
		self.q_values = np.zeros((20, 7, 3))
		...
		self.position=0#armazena a posição do tetromino
		self.old_action =-1
	...
	def update_q_value(self):
		...
		self.q_values[old_h_mean, old_tetromino, self.old_action] = new_q_value
